...
static void update_bt_mac(uint8_t *addr, bool random) {
    ...
    if (random) {
        ...
        if (force_random != 1 && stat(VENDOR_BT_MAC_PATH, &st) == 0 && st.st_size >= MAC_SIZE) {
    ...
    fwrite(addr, sizeof(uint8_t), MAC_SIZE, fd);
    fclose(fd);
...
static void get_mac_from_persist() {
    struct stat st;
    FILE *fd;
    uint8_t *buf = (uint8_t *) malloc(MAC_SIZE);
    int len = 0;
    bool random = false;
    ...
    if (!random) {
        if (fscanf(fd, "%02X:%02X:%02X:%02X:%02X:%02X", buf, buf + 1, buf + 2, buf + 3, buf + 4, buf + 5) != MAC_SIZE) {
